...
{
    var len = array_length(global.inv);
    if (_i < 0 || _i >= len) return false;

    global.inv[_i].qty -= _amount;

    if (global.inv[_i].qty <= 0)
    {
        // manual delete: shift left, then shrink
        for (var j = _i; j < len - 1; j++)
        {
            global.inv[j] = global.inv[j + 1];
        }
        array_resize(global.inv, len - 1);
    }
	show_debug_message("len_after=" + string(array_length(global.inv)));
    return true;
}
...
function inv_remove_id(_id, _amount)
{
    for (var i = 0; i < array_length(global.inv); i++)
    {
        if (global.inv[i].id == _id)
        {
            // remove in one go from that stack
